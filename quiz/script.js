...
// Retorna o valor do RadioButton selecionado
function valorResposta(){
  var resposta;
...
// Valida a resposta, e incrementa a pontuação
function validaResposta(resposta){
  var valido = false;
  var questao = questoes[indiceQuestaoAtual];
  if(resposta == questao.resposta){
    pontuacao++;
    tocarAudio('/sound/level-up.mp3');
    valido = true;
  }
  else{
    tocarAudio('/sound/wrong.mp3');
  }
  return valido;
}

// Toca um audio dependendo da ação
function tocarAudio(localAudio){
  var audio = new Audio(localAudio);
  audio.addEventListener('canplaythrough', function() {
  audio.play();
  });
}

// Pega a proxima questão da lista, caso houver
function proximaQuestao(){
  if(indiceQuestaoAtual < questoes.length -1){
    ...
    desabilitaQuestionario();
    tocarAudio('/sound/game-over.mp3');
    delayAlert('Sua pontuação foi de '+ pontuacao, 100);
  }
}

// Alerta com Delay (útil para ser executado com os sons :D)
...
function processarResposta() {
  ...
  if(resposta){
    ...
    tocarAudio('/sound/fail.mp3');
    
    delayAlert('É necessário selecionar uma alternativa.', 100);
  }
  // TODO (implementar)
}
